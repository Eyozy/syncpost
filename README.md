...
> 附件超过 10MB 时会直接拒绝，不会直接发布。图片超过 4 张时同样会拒绝（Mastodon 限制）。
...
直接向机器人发送纯文本、图片或相册（支持直接发图或以文件形式发送）：
...
- 不支持 GIF 图、视频、音频、语音、贴纸、视频留言，单个附件不能超过 10MB
- 编辑带附件的消息时，只会更新文字说明（caption），不会替换附件本身
